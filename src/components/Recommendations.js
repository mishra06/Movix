...
import { SetRecomend } from '../slice/movieSlice';
...
const Recommendations = () => {
    ...
    const TopRateMovies = useSelector((state) => state.movieSlice.Recomend);
    
      const dispatch = useDispatch();
    
      
      async function GetData() {
        const data = await fetchApi(`https://api.themoviedb.org/3/movie/${id}/recommendations`, {
          headers: {
        ...
        dispatch(SetRecomend(data.data.results));
      }
      ...
      return (
        <div style={{width:"100%",display:"flex",justifyContent:"center",alignItems:"center"}} className="">
            ...
            <div style={{color:"white",display:"flex",justifyContent:"space-between",width:"80%",marginTop:"5px"}}>
                <div><h2 style={{fontSize:"35px" , fontWeight:"600"}}>Recommendations</h2></div>
            </div>
          <ReactSimplyCarousel
            style={{border:"none"}}
